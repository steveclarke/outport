...
- **Local config overrides** — `outport.local.yml` (not committed) merges on top of `outport.yml` at load time. Only services already in the base config can be overridden — field-level merge where non-zero/non-empty local values win. Aliases replace entirely (map swap, not key-by-key). Project name and computed values in the local file are ignored. Validation runs on the merged result.
