...
- **`.test` hostnames** — Services with a `hostname` get `.test` domain hostnames (e.g., `myapp.test`). Non-main instances get suffixed hostnames (e.g., `myapp-bxcf.test`). Hostnames are globally unique across all registered projects.
- **Template expansion** — Computed values use bash-style parameter expansion. Service fields: `${service.port}`, `${service.hostname}`, `${service.url}`, `${service.url:direct}`, `${service.env_var}`. Standalone variables: `${instance}` (empty for main, instance code for worktrees), `${project_name}` (project name from config). Operators: `${var:-default}` (use default if empty), `${var:+replacement}` (use replacement if non-empty). Example: `"${project_name}${instance:+-${instance}}"` → `myapp` for main, `myapp-xbjf` for worktrees.
