...
- **Instance model** — The first checkout of a project is "main". Additional checkouts (worktrees, clones) get auto-generated 4-character codes. Instances can be renamed (`outport rename`) or promoted to main (`outport promote`). The registry is the source of truth for instance identity — directories are matched by filesystem identity (`os.SameFile`, device+inode), so the same directory resolves to the same instance regardless of path spelling or case (fixes phantom instances on case-insensitive filesystems). `outport system prune` removes duplicate-directory phantoms left by older versions.
- **`.test` hostnames** — Services declare `.test` hostnames explicitly (e.g., `hostname: myapp.test`). The `.test` suffix is required in config. Non-main instances get suffixed hostnames (e.g., `myapp-bxcf.test`). Hostnames are globally unique across all registered projects. Services can also define named `aliases` (a map of label → hostname) that register additional proxy routes to the same port — each alias follows the same validation rules as primary hostnames and gets the same instance suffix treatment for non-main instances. Services can also enable `subdomains: true` for wildcard subdomain routing — all subdomains of the primary hostname route to the same port. Exact matches (other services or aliases) take precedence over wildcards.
