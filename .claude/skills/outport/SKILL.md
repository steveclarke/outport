...
outport share             # Tunnel HTTP services to public URLs (Cloudflare)

# .test domain routing (macOS, one-time setup)
outport system start      # Install DNS resolver, proxy daemon, and CA
outport system stop       # Stop the daemon
outport system restart    # Restart daemon (updates binary path after upgrades)
...
## .test Domains and HTTPS

Running `outport system start` once enables friendly `.test` hostnames with
automatic HTTPS for your services. After setup, `https://myapp.test` routes
to your app instead of `http://localhost:24920`.

### How it works

`outport system start` installs three components (macOS, requires sudo for DNS):

1. **DNS resolver** — `/etc/resolver/test` points `*.test` queries to a
   local DNS server on port 15353, which resolves all `*.test` names to
   `127.0.0.1`.
2. **HTTP reverse proxy** — a LaunchAgent runs on port 80, issues 307
   redirects to HTTPS when the CA is installed.
3. **TLS reverse proxy** — runs on port 443, terminates TLS using
   auto-generated per-hostname certificates, proxies to your backend over
   plain HTTP. WebSocket connections are proxied transparently.
4. **Local CA** — an EC P-256 certificate authority trusted in the macOS
   keychain. Server certs are generated on demand via TLS SNI callback.

```bash
outport system start      # Install DNS + daemon + CA (one-time, prompts for sudo)
outport system uninstall  # Remove everything — reverse of start
outport system stop       # Stop the daemon
outport system restart    # Restart daemon (rewrites plist with current binary path)
```
...
services:
  web:
    env_var: PORT
    protocol: http
    hostname: myapp         # → https://myapp.test (with CA) or http://myapp.test
  postgres:
    env_var: DB_PORT        # no hostname: port allocation only
```

Hostname rules:
- Must include the project name (e.g., `myapp`, `app.myapp`, `portal.myapp`)
- The `.test` suffix is added automatically — don't include it
...
```text
myapp [main]   web → 24920   https://myapp.test
myapp [bkrm]   web → 28104   https://myapp-bkrm.test
```
...
| `hostname` | no | `.test` hostname for this service (e.g., `myapp`). Requires `protocol: http` or `https`. Non-main instances get the instance code appended. |
...
derived:
  ...
  CORS_ORIGINS:
    value: "${web.url}"                   # https://myapp.test
...
| `${rails.url}` | `https://myapp.test` (or `http://` without CA) | Browser-facing URLs (CORS, asset hosts), routed via proxy |
...
- `${service.url}` — for values the browser sends (CORS origins, asset
  ...
  Produces `https://` when the CA is installed.
...
derived:
  ...
  # Docker Compose isolation per instance
  COMPOSE_PROJECT_NAME:
    value: "myapp${instance:+-${instance}}"
    env_file: backend/.env
```
...
$ outport up
my-app [main]
  rails  RAILS_PORT → 24920  https://my-app.test
  web    MAIN_PORT  → 21349
...
my-app [bkrm]
  rails  RAILS_PORT → 20192  https://my-app-bkrm.test
  web    MAIN_PORT  → 21133
```
...
`outport system start` was never run, run it now. If the daemon is
stopped, run `outport system restart`.

### Sharing services publicly
Run `outport share` to tunnel all HTTP services via Cloudflare. Requires
