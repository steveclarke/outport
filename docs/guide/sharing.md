...
Note that [wildcard subdomain routing](/reference/configuration#subdomains) (`subdomains: true`) is local-only — tunnels use explicit hostnames, not wildcards. If you need a specific subdomain reachable through a tunnel, add it as an explicit alias.
