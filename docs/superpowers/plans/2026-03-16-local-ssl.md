...
- In `resolveDerivedFromAlloc` (apply.go): `buildTemplateVars(cfg, ports, hostnames, certmanager.IsCAInstalled())`
...
- In `runApply` where `mergeEnvFiles` is called: the merge uses `resolveDerivedFromAlloc` which already calls `buildTemplateVars`
